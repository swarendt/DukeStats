...
```{r streak-kobe-analysis}

kobe_streak <- calc_streak(kobe_basket$shot)
IQR(kobe_streak$length)
max(kobe_streak$length)
min(kobe_streak$length)
...
The simulations will be similar.  
...
The answer to question 5 is that the distributions are similar.  The best way to interpret this is that since a 45% shooting rate is similar to Kobe's distribution.  Since we have been told that 45% is not considered a hot hand, and that the distributions are similar, then that means that Kobe is not demonstrating a hot hand in his results.  The first option is the correct answer.
